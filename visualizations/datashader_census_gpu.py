...
spark.read.parquet('/ml/census2010.parq').count()
...
# MAGIC %md ### Interactive HoloViews+Bokeh plots overlaid with map data
# MAGIC
# MAGIC The above plots all show static images on their own.  datashader is independent of Bokeh, HoloViews, or other visualization libraries, but when combined with HoloViews and Bokeh, it is easy to make interactive plots that incorporate maps, satellite imagery, annotations, legends, and hover-tool information.
# MAGIC
# MAGIC To do this, let's import what we need and set some defaults:
...
# MAGIC %md And then create an interactive plot using a dynamic wrapper ``datashade`` that calls datashader whenever someone zooms or pans the plot.  In this pipeline, we'll use the `tf.dynspread` function to automatically increase the plotted size of each datapoint, once you've zoomed in so far that datapoints no longer have nearby neighbors.  We'll also add some image tiles in the background, using satellite-derived maps:
...
# MAGIC %md Note that you need a live copy of the notebook, with a running server; zooming and panning will be disabled (or only work partially, which can be confusing!) in a static exported copy like on anaconda.org.
# MAGIC
# MAGIC You can similarly zoom into the population density data (ignoring race):

# COMMAND ----------

labelsurl="http://tile.stamen.com/toner-labels/{Z}/{X}/{Y}.png"
labels = gv.WMTS(WMTSTileSource(url=labelsurl))
...
hv.HoloMap(population)

# COMMAND ----------

# MAGIC %md The dashboard.py example shows this same Census data in the context of an interactive dashboard, including color keys and hover information that help reveal the magnitudes at every location even while the plot faithfully reveals the structure.
